...
int main(int argc, char** argv)
{
    ...
    //  Override size from argv
    int WA = 2048, HA = 2048;
    if (argc > 1) {
        WA = atoi(argv[1]);
        HA = WA;
    }
    int WB = WA, HB = HA, WC = WA, HC = HA;
    ...
    FILE *fp;
	const char fileName[] = "src/matrixmul_kernel.cl";
	size_t kernel_size;
	char *KernelSource;
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(int), &R1);
    err |= clSetKernelArg(kernel, 4, sizeof(int), &C1);
    err |= clSetKernelArg(kernel, 5, sizeof(int), &R2);
    err |= clSetKernelArg(kernel, 6, sizeof(int), &C2);
	if (err != CL_SUCCESS){
    ...
    err = clEnqueueReadBuffer(commands, d_C, CL_TRUE, 0, sizeof(float)*HC*WC, h_C, 0, NULL, NULL);
	if (err != CL_SUCCESS){
